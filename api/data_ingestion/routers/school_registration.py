...
def _build_extra_fields(
    ...
    """Store registration-specific fields in column_to_schema_mapping."""
    return {
        "school_id_giga": payload.giga_id_school,
        "school_id_govt": payload.school_id,
...
async def trigger_registration_pipeline(
    ...
    existing = await db.scalar(
        select(FileUpload)
        .where(
            FileUpload.column_to_schema_mapping["school_id_giga"].as_string()
            == payload.giga_id_school,
    ...
    file_upload = FileUpload(
        ...
        column_to_schema_mapping=_build_extra_fields(payload),
        column_license={},
    )
    ...
    try:
        write_registration_csv_to_adls(payload.model_dump(), file_upload)
...
async def retrigger_registration_pipeline(
    ...
    try:
        write_registration_csv_to_adls(payload.model_dump(), new_file_upload)
    except Exception as err:
